...
```bash
$ bitad tx gov vote 16 yes --from <tipper_address> --aux --tip 50ibcdenom
...
```bash
$ bitad tx aux-to-fee aux_signed_tx.json --from <fee_payer_address> --fees 30BITA
...
Upon completion of the second command, the fee payer's balance will be down the `30BITA` fees, and up the `50ibcdenom` tip.
