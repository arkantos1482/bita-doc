...
The `grpc.Server` is a concrete gRPC server, which spawns and serves all gRPC query requests and a broadcast transaction request. This server can be configured inside `~/.bita/config/app.toml`:

* `grpc.enable = true|false` field defines if the gRPC server should be enabled. Defaults to `true`.
* `grpc.address = {string}` field defines the `ip:port` the server should bind to. Defaults to `localhost:9090`.

:::tip
`~/.bita` is the directory where the node's configuration and databases are stored. By default, it's set to `~/.{app_name}`.
:::
...
All routes are configured under the following fields in `~/.bita/config/app.toml`:
...
* some additional API configuration options are defined in `~/.bita/config/app.toml`, along with comments, please refer to that file directly.
...
Enabling the `/swagger` endpoint is configurable inside `~/.bita/config/app.toml` via the `api.swagger` field, which is set to true by default.
