...
Here is an example of a command a user might enter to interact with the simapp CLI `bitad` in order to send some tokens:

```bash
bitad tx bank send $MY_VALIDATOR_ADDRESS $RECIPIENT 1000stake --gas auto --gas-prices <gasPrices>
```

The first four strings specify the command:

* The root command for the entire application `bitad`.
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.50.0-alpha.0/simapp/bitad/main.go#L12-L24
```
...
The root command (called `rootCmd`) is what the user first types into the command line to indicate which application they wish to interact with. The string used to invoke the command (the "Use" field) is typically the name of the application suffixed with `-d`, e.g. `bitad` or `bitad`. The root command typically includes the following commands to support basic functionality in the application.
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.50.0-alpha.0/simapp/bitad/cmd/root_v2.go#L47-L130
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.50.0-alpha.0/simapp/bitad/cmd/root_v2.go#L144-L199
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.50.0-alpha.0/simapp/bitad/cmd/root_v2.go#L164-L180
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.50.0-alpha.0/simapp/bitad/cmd/root_v2.go#L222-L229
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.50.0-alpha.0/simapp/bitad/cmd/root_v2.go#L270-L292
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.50.0-alpha.0/simapp/bitad/cmd/root_v2.go#L249-L268
```
...
Each flag is bound to it's respecteve named environment variable. Then name of the environment variable consist of two parts - capital case `basename` followed by flag name of the flag. `-` must be substituted with `_`. For example flag `--home` for application with basename `BITA` is bound to `BITA_HOME`. It allows reducing the amount of flags typed for routine operations. For example instead of:

```shell
bita --home=./ --node=<node address> --chain-id="testchain-1" --keyring-backend=test tx ... --from=<key name>
```
...
BITA_HOME=<path to home>
BITA_NODE=<node address>
BITA_CHAIN_ID="testchain-1"
BITA_KEYRING_BACKEND="test"

# and later just use
bita tx ... --from=<key name>
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/blob/v0.50.0-alpha.0/simapp/bitad/cmd/root_v2.go#L81-L120
```
