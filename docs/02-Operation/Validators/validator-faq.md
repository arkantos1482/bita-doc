...
Delegators are BITA holders who cannot, or do not want to, run a validator themselves. BITA holders can delegate BITA to a validator and obtain a part of their revenue in exchange. For details on how revenue is distributed, see [What is the incentive to stake?](#what-is-the-incentive-to-stake?) and [What are validators commission?](#what-are-validators-commission?) in this document.
...
- **Minimum self-delegation:** Minimum amount of BITA the validator requires to have bonded at all time. If the validator's self-delegated stake falls below this limit, their validator gets jailed and kicked out of the active validator set.

After a validator is created, BITA holders can delegate BITA to them, effectively adding stake to the validator's pool. The total stake of an address is the combination of BITA bonded by delegators and BITA self-bonded by the validator.
...
- **Tendermint/CometBFT key**: A unique key that is used to sign consensus votes.
  - It is associated with a public key `cosmosvalconspub` (To get this value, run `bitad tendermint show-validator`)
  - It is generated when the node is created with `bitad init`.
- **Application key**: This key is created from the `bitad` binary and is used to sign transactions. Application keys are associated with a public key that is prefixed by `cosmospub` and an address that is prefixed by `cosmos`.

The Tendermint/CometBFT key and the application key are derived from account keys that are generated by the `bitad keys add` command.
...
- `unbonded`: Validator is not in the active set, and therefore not signing blocks. The validator cannot be slashed and does not earn any reward. It is still possible to delegate BITA to an unbonded validator. Undelegating from an `unbonded` validator is immediate, meaning that the tokens are not subject to the unbonding period.

### What is self-delegation? How can I increase my self-delegation?

Self-delegation is a delegation of BITA from a validator to themselves. The delegated amount can be increased by sending a `delegate` transaction from your validator's `application` application key.

### Is there a minimum amount of BITA that must be delegated to be an active (bonded) validator?

The minimum is 1 BITA. But the network is currently secured by much higher values. You can check the minimum required BITA to become part of the active validator set on the [Mintscan validator page](https://www.mintscan.io/cosmos/validators).
...
- **Amount of self-delegated BITA:** Number of BITA a validator self-delegated to themselves. A validator with a higher amount of self-delegated BITA indicates that the validator is sharing the risk and experienced consequences for their actions.
- **Amount of delegated BITA:** Total number of BITA delegated to a validator. A high voting power shows that the community trusts this validator. Larger validators also decrease the decentralization of the network, so delegators are suggested to consider delegating to smaller validators.
...
Staking BITA can be thought of as a safety deposit on validation activities. When a validator or a delegator wants to retrieve part or all of their deposit, they send an `unbonding` transaction. Then, BITA undergoes a **3-week unbonding period** during which they are liable to being slashed for potential misbehaviors committed by the validator before the unbonding process started.

Validators, and by association delegators, receive block rewards, fees, and have the right to participate in governance. If a validator misbehaves, a certain portion of their total stake is slashed. This means that every delegator that bonded BITA to this validator gets penalized in proportion to their bonded stake. Delegators are therefore incentivized to delegate to validators that they anticipate will function safely.

### Can a validator run away with their delegators' BITA?

By delegating to a validator, a user delegates voting power. The more voting power a validator have, the more weight they have in the consensus and governance processes. This does not mean that the validator has custody of their delegators' BITA. **A validator cannot run away with its delegator's funds**.
...
### How often is a validator chosen to propose the next block? Does frequency increase with the quantity of bonded BITA?

The validator that is selected to propose the next block is called the proposer. Each proposer is selected deterministically. The frequency of being chosen is proportional to the voting power (i.e. amount of bonded BITA) of the validator. For example, if the total bonded stake across all validators is 100 BITA and a validator's total stake is 10 BITA, then this validator is the proposer ~10% of the blocks.
...
- **Block rewards:** Native tokens of applications (e.g. BITA on the Cosmos Hub) run by validators are inflated to produce block provisions. These provisions exist to incentivize BITA holders to bond their stake. Non-bonded BITA are diluted over time.
- **Transaction fees:** The Cosmos Hub maintains an allow list of tokens that are accepted as fee payment. The initial fee token is the `BITA`.
...
Block rewards are distributed proportionally to all validators relative to their voting power. This means that even though each validator gains BITA with each reward, all validators maintain equal weight over time.

For example, 10 validators have equal voting power and a commission rate of 1%. For this example, the reward for a block is 1000 BITA and each validator has 20% of self-bonded BITA. These tokens do not go directly to the proposer. Instead, the tokens are evenly spread among validators. So now each validator's pool has 100 BITA. These 100 BITA are distributed according to each participant's stake:

- Commission: `100*80%*1% = 0.8 BITA`
- Validator gets: `100\*20% + Commission = 20.8 BITA`
- All delegators get: `100\*80% - Commission = 79.2 BITA`

Then, each delegator can claim their part of the 79.2 BITA in proportion to their stake in the validator's staking pool.
...
For a concrete example to illustrate the aforementioned concept, there are 10 validators with equal stake. Each validator applies a 1% commission rate and has 20% of self-delegated BITA. Now comes a successful block that collects a total of 1025.51020408 BITA in fees.

First, a 2% tax is applied. The corresponding BITA go to the reserve pool. The reserve pool's funds can be allocated through governance to fund bounties and upgrades.

- `2% * 1025.51020408 = 20.51020408` BITA go to the reserve pool.

1005 BITA now remain. For this example, the proposer included 100% of the signatures in its block so the proposer obtains the full bonus of 5%.
...
- For the proposer validator:
  - The pool obtains `R + R * 5%`: 105 BITA
  - Commission: `105 * 80% * 1%` = 0.84 BITA
  - Validator's reward: `105 * 20% + Commission` = 21.84 BITA
  - Delegators' rewards: `105 * 80% - Commission` = 83.16 BITA (each delegator is able to claim its portion of these rewards in proportion to their stake)
- For each non-proposer validator:
  - The pool obtains R: 100 BITA
  - Commission: `100 * 80% * 1%` = 0.8 BITA
  - Validator's reward: `100 * 20% + Commission` = 20.8 BITA
  - Delegators' rewards: `100 * 80% - Commission` = 79.2 BITA (each delegator is able to claim their portion of these rewards in proportion to their stake)
...
### Are validators required to self-delegate BITA?

Yes, they do need to self-delegate at least `1 BITA`. Even though there is no obligation for validators to self-delegate more than `1 BITA`, delegators want their validator to have more self-delegated BITA in their staking pool. In other words, validators share the risk.

In order for delegators to have some guarantee about how much shared risk their validator has, the validator can signal a minimum amount of self-delegated BITA. If a validator's self-delegation goes below the limit that it predefined, the validator gets jailed and kicked out of the active set of validators while its delegators remain bonded to it.
