...
The last step is to retrieve the validator key that you will use in `bitad`.
...
The KMS may complain that it cannot connect to bitad. That is fine, we will fix it in the next section.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell bitad the key that we've got in the previous section.

```bash
bitad gentx --name kmsval --pubkey {.ValidatorKey} 
```

Now start `bitad`. You should see that the KMS connects and receives a signature request.
