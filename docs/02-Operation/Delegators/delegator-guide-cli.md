...
loss of your BITAs. Therefore, read through the following instructions in their
...
- [Installing `bitad`](#installing-bitad)
...
- [Setting Up `bitad`](#setting-up-bitad)
- [Querying the State](#querying-the-state)
- [Sending Transactions](#sending-transactions)
  - [A Note on Gas and Fees](#a-note-on-gas-and-fees)
  - [Bonding BITAs and Withdrawing Rewards](#bonding-BITAs-and-withdrawing-rewards)
  - [Participating in Governance](#participating-in-governance)
  - [Signing Transactions from an Offline Computer](#signing-transactions-from-an-offline-computer)

## Installing `bitad`

`bitad`: This is the command-line interface to interact with a `bitad` full-node.

:::warning
**Please check that you download the latest stable release of `bitad` that is available**
:::
...
:::tip
`bitad` is used from a terminal. To open the terminal, follow these steps:
...
```bash
bitad keys add <yourKeyName> --recover
```
...
To create an account, you just need to have `bitad` installed. Before creating it, you need to know where you intend to store and interact with your private keys. The best options are to store them in an offline dedicated computer or a ledger device. Storing them on your regular online computer involves more risk, since anyone who infiltrates your computer through the internet could exfiltrate your private keys and steal your funds.
...
When you initialize your ledger, a 24-word mnemonic is generated and stored in the device. This mnemonic is compatible with Cosmos and Cosmos accounts can be derived from it. Therefore, all you have to do is make your ledger compatible with `bitad`. To do so, you need to go through the following steps:
...
```bash
bitad keys add <yourAccountName> --ledger
```
...
```bash
bitad keys add <yourKeyName> --recover --account 1
```
...
You will find the tutorial on how to install `bitad` [here](../getting-started/installation.md), and the guide to run a full-node [here](../hub-tutorials/join-mainnet.md).

### Connecting to a Remote Full-Node

If you do not want or cannot run your own node, you can connect to someone else's full-node. You should pick an operator you trust, because a malicious operator could return  incorrect query results or censor your transactions. However, they will never be able to steal your funds, as your private keys are stored locally on your computer or ledger device. Possible options of full-node operators include validators, wallet providers or exchanges.

In order to connect to the full-node, you will need an address of the following form: `https://77.87.106.33:26657` (*Note: This is a placeholder*). This address has to be communicated by the full-node operator you choose to trust. You will use this address in the [following section](#setting-up-bitad).

## Setting Up `bitad`

:::tip
**Before setting up `bitad`, make sure you have set up a way to [access the Cosmos Hub network](#accessing-the-cosmos-hub-network)**
:::

:::warning
**Please check that you are always using the latest stable release of `bitad`**
:::

`bitad` is the tool that enables you to interact with the node that runs on the Cosmos Hub network, whether you run it yourself or not. Let us set it up properly.

In order to set up `bitad`, use the following command:

```bash
bitad config <flag> <value>
```
...
```bash
bitad config node <host>:<port

// example: bitad config node https://77.87.106.33:26657 (note: this is a placeholder)
```
...
```bash
bitad config chain-id cosmoshub-2
```

## Querying the State

:::tip
**Before you can bond BITAs and withdraw rewards, you need to [set up `bitad`](#setting-up-bitad)**
:::

`bitad` lets you query all relevant information from the blockchain, like account balances, amount of bonded tokens, outstanding rewards, governance proposals and more. Next is a list of the most useful commands for delegator.

```bash
// query account balances and other account-related information
bitad query account <yourAddress>

// query the list of validators
bitad query staking validators

// query the information of a validator given their address (e.g. cosmosvaloper1n5pepvmgsfd3p2tqqgvt505jvymmstf6s9gw27)
bitad query staking validator <validatorAddress>

// query all delegations made from a delegator given their address (e.g. cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg)
bitad query staking delegations <delegatorAddress>
...
bitad query staking delegation <delegatorAddress> <validatorAddress>

// query the rewards of a delegator given a delegator address (e.g. cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg)
bitad query distribution rewards <delegatorAddress>

// query all proposals currently open for depositing
bitad query gov proposals --status deposit_period

// query all proposals currently open for voting
bitad query gov proposals --status voting_period

// query a proposal given its proposalID
bitad query gov proposal <proposalID>
```
...
:::warning
On Cosmos Hub mainnet, the accepted denom is `uBITA`, where `1BITA = 1,000,000uBITA`
:::
...
:::tip
For mainnet, the recommended `gas-prices` is `0.0025uBITA`.
:::

### Sending Tokens

:::tip
**Before you can bond BITAs and withdraw rewards, you need to [set up `bitad`](#setting-up-bitad) and [create an account](#creating-an-account)**
:::
...
// Ex value for parameters (do not actually use these values in your tx!!): <to_address>=cosmos16m93fezfiezhvnjajzrfyszml8qm92a0w67ntjhd3d0 <amount>=1000000uBITA
// Ex value for flags: <gasPrice>=0.0025uBITA

bitad tx bank send [from_key_or_address] [to_address] [amount] [flags]
```

### Bonding BITAs and Withdrawing Rewards
...
:::warning
**Before bonding BITAs, please read the [delegator faq](https://cosmos.network/resources/delegators) to understand the risk and responsibilities involved with delegating**
:::
...
```bash
// Bond a certain amount of BITAs to a given validator
// ex value for flags: <validatorAddress>=cosmosvaloper18thamkhnj9wz8pa4nhnp9rldprgant57pk2m8s, <amountToBound>=10000000uBITA, <gasPrice>=0.0025uBITA

bitad tx staking delegate <validatorAddress> <amountToBond> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>


// Redelegate a certain amount of BITAs from a validator to another
// Can only be used if already bonded to a validator
// Redelegation takes effect immediately, there is no waiting period to redelegate
// After a redelegation, no other redelegation can be made from the account for the next 3 weeks
// ex value for flags: <stcValidatorAddress>=cosmosvaloper18thamkhnj9wz8pa4nhnp9rldprgant57pk2m8s, <amountToRedelegate>=100000000uBITA, <gasPrice>=0.0025uBITA

bitad tx staking redelegate <srcValidatorAddress> <destValidatorAddress> <amountToRedelegate> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>

// Withdraw all rewards
// ex value for flag: <gasPrice>=0.0025uBITA

bitad tx distribution withdraw-all-rewards --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>


// Unbond a certain amount of BITAs from a given validator
// You will have to wait 3 weeks before your BITAs are fully unbonded and transferrable
// ex value for flags: <validatorAddress>=cosmosvaloper18thamkhnj9wz8pa4nhnp9rldprgant57pk2m8s, <amountToUnbound>=10000000uBITA, <gasPrice>=0.0025uBITA

bitad tx staking unbond <validatorAddress> <amountToUnbond> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
```
...
```bash
// your balance should change after you bond BITAs or withdraw rewards
bitad query account

// you should have delegations after you bond BITA
bitad query staking delegations <delegatorAddress>

// this returns your tx if it has been included
// use the tx hash that was displayed when you created the tx
bitad query tx <txHash>
...
The Cosmos Hub has a built-in governance system that lets bonded BITA holders vote on proposals. There are three types of proposal:
...
Any BITA holder can submit a proposal. In order for the proposal to be open for voting, it needs to come with a `deposit` that is greater than a parameter called `minDeposit`. The `deposit` need not be provided in its entirety by the submitter. If the initial proposer's `deposit` is not sufficient, the proposal enters the `deposit_period` status. Then, any BITA holder can increase the deposit by sending a `depositTx`.

Once the `deposit` reaches `minDeposit`, the proposal enters the `voting_period`, which lasts 2 weeks. Any **bonded** BITA holder can then cast a vote on this proposal. The options are `Yes`, `No`, `NoWithVeto` and `Abstain`. The weight of the vote is based on the amount of bonded BITAs of the sender. If they don't vote, delegator inherit the vote of their validator. However, delegators can override their validator's vote by sending a vote themselves.
...
:::tip
**Before you can bond BITAs and withdraw rewards, you need to [bond BITAs](#bonding-BITAs-and-withdrawing-rewards)**
:::
...
bitad tx gov submit-proposal --title "Test Proposal" --description "My awesome proposal" --type <type> --deposit=10000000uBITA --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>

// Increase deposit of a proposal
// Retrieve proposalID from $bitad query gov proposals --status deposit_period
// ex value for parameter: <deposit>=10000000uBITA

bitad tx gov deposit <proposalID> <deposit> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>

// Vote on a proposal
// Retrieve proposalID from $bitad query gov proposals --status voting_period
// <option>=yes/no/no_with_veto/abstain

bitad tx gov vote <proposalID> <option> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>
```
...
```bash
// Bond BITAs
// ex value for flags: <amountToBound>=10000000uBITA, <bech32AddressOfValidator>=cosmosvaloper18thamkhnj9wz8pa4nhnp9rldprgant57pk2m8s, <gasPrice>=0.0025uBITA, <delegatorAddress>=cosmos10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg

bitad tx staking delegate <validatorAddress> <amountToBond> --from <delegatorAddress> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --generate-only > unsignedTX.json
```
...
```bash
bitad query account <yourAddress> --chain-id cosmoshub-2
```
...
```bash
bitad tx sign unsignedTx.json --from <delegatorKeyName> --offline --chain-id cosmoshub-2 --sequence <sequence> --account-number <account-number> > signedTx.json
```

Copy `signedTx.json` and transfer it back to the online computer. Finally, use the following command to broadcast the transaction:

```bash
bitad tx broadcast signedTx.json
```
